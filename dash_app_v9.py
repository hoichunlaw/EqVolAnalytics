...
def loadConfig_HKStockOptions():
    
    url = "https://www.etnet.com.hk/www/eng/futures/futures_stockoptions.php"
    page = requests.get(url)
    soup = BeautifulSoup(page.text, "html")
    
    tags = soup.find("select", id="atscodelist")
    tags = tags.find_all("option")
    
    undlDict = {}
    for tag in tags:
        text = tag.text
        value = tag["value"]
        undlName = text.split(" - ")[0][1:]+".HK"
        
        undlDict[undlName] = value
        
    tags = soup.find("select", id="monthlist")
    tags = tags.find_all("option")
    
    monthList = []
    for tag in tags:
        monthList.append(tag["value"])
        
    return undlDict, monthList

def getOptionChain_HKStock_singleMaturity(undlCode, maturityCode):
    
    url = "https://www.etnet.com.hk/www/eng/futures/futures_stockoptions.php?atscode="+undlCode+"&month="+maturityCode
    page = requests.get(url)
    soup = BeautifulSoup(page.text, "html")
    
    contents = soup.find_all("div", class_="DivFigureContent")
    spotRef = float(contents[0].find_all("span")[1].text)

    contents = soup.find_all("div", class_="DivFigureContent")
    trs = contents[1].find_all("tr")[3].find_all("tr")
    
    options = []
    for tr in trs:
        tds = tr.find_all("td")
        if tds[3].text != "" and tds[4].text != "":
            if float(tds[6].text) >= spotRef:
                call = {"maturity": maturityCode, "strike": float(tds[6].text), "optionType": "C", 
                        "bid": float(tds[3].text), "ask": float(tds[4].text),}        
                options.append(call)
            
        if tds[8].text != "" and tds[9].text != "":
            if float(tds[6].text) <= spotRef:
                put = {"maturity": maturityCode, "strike": float(tds[6].text), "optionType": "P", 
                        "bid": float(tds[8].text), "ask": float(tds[9].text),}        
                options.append(put)
            
    return options, spotRef

def getHK_SQDate(year, month):
    
    lastDayOfMonth = calendar.monthrange(int(year), int(month))[1]
    
    firstDayOfNextMonth = datetime(int(year), int(month), int(lastDayOfMonth)) + timedelta(days=1)
    firstDayOfNextMonth = firstDayOfNextMonth.strftime("%Y-%m-%d")
    
    result = get_nextBusinessDay(firstDayOfNextMonth, -2, "HKEX")
...
def getOptionChain_HKStock(undlName):
    
    valueDate = get_exchangeDate(get_calendar(undlName))
    
    undlDict, monthList = loadConfig_HKStockOptions()
    
    undlCode = undlDict[undlName]
    
    optionChain = []
    for month in monthList:
        tmp, spotRef = getOptionChain_HKStock_singleMaturity(undlCode, month)
        for t in tmp:
            year = t["maturity"][:4]
            mth = t["maturity"][4:]
            expiryDate = getHK_SQDate(year, mth)
            if expiryDate is not None:
                option = {"maturity": expiryDate, "strike": t["strike"], "optionType": t["optionType"],
                         "optionStyle": "American", "valueDate": valueDate,
                         "bid": t["bid"], "ask": t["ask"]}
                optionChain.append(option)
                
    optionChainData = {"undlName": undlName, 
                       "spotRef": spotRef,
...
tableUdlInfo = dag.AgGrid(id="tableUndlInfo",
                          ...
                          columnDefs=[{"field": "Field", "minWidth": 120, "maxWidth": 120, "pinned": "left"}, 
                                      {"field": "Value", "maxWidth": 150, "minWidth": 150, "editable": False}], 
                          columnSize="autoSize",
...
modalLogin = dbc.Modal(
    [
        ...
        dbc.ModalBody([html.H6("User Name"),
                       dcc.Input(id="input-username", placeholder=""),
                       html.H6(" "),
                       html.H6("Password"),
                       dcc.Input(id="input-password", placeholder=""),
                       html.H6(" ")]),
        dbc.ModalFooter([dbc.Button("Login", id="button-modal-login", className="me-2", outline=True, color="info", n_clicks=0)])
    ],
    id="modal-login",
    centered=True,
    is_open=False
...
sidebar = html.Div(
    [   
        ...
        html.Div([
            ...
            html.Div(html.H6(" "), style={"height": "75px", "width": "100%"}),
        ...
        html.Div(
            [
                ...
                html.Div(" "),
                html.Div(html.Div("Login as:", id="text-username"))
            ]
        ),
    ],
    style=SIDEBAR_STYLE,
...
def createContentSetting():
    ...
    topBar = html.Div([html.Div([dbc.Button("Save", id="button-saveSetting", outline=True, color="danger", className="me-2", n_clicks=0)], style={'display': 'inline-block'}),
                        html.H6(" "),
                        html.Div("Status:", id="text-settingPageStatus"),
                        html.Hr()])
   
    page = html.Div([html.H6("New Password:"), 
                     #dcc.Dropdown(["True", "False"], value="True" if useBBGData == 1 else "False", id='dropdown-BBGData', style={'color': '#999999'}),
                     dcc.Input(id="text-settingPassword"),
                     html.H6(" "),
                     html.H6("Confirm Password:"), 
                     dcc.Input(id="text-settingPasswordConfirm"),
                     html.H6(" "),
...
def SVISToSVIJW(paramsSVIS, maturities, username="None"):
...
def createContentVolatility(undlName: str, memoryData: dict, dataCheck: bool=True):
    ...
    panelParams = html.Div([html.H6(f"Anchor: {volSurfaceSVI['anchor']}", id="text-anchor"), 
                            ...
                            dbc.Row([html.Div(dbc.Collapse(paramTable, id="collapse-SVI", is_open=True), style={"width": 705, "height": 450, "margin-right":"5px"}),
                                    ...
                                    html.Div(dbc.Collapse(refParamTable, id="collapse-refSVI", is_open=False), style={"width":525, "height": 450, "margin-left": "5px"})])], 
    ...
    subPanelMaturity = html.Div([dbc.Row(dcc.Dropdown(maturities, value=maturities[0], id='maturity-dropdown', style={'color': '#999999', "margin-top": "30px", "margin-bottom": "5px"})),
                                ...
                                html.Div(dbc.Collapse([paramsSubTable, 
                                                       paramsSubTable2,
                                                       dbc.Button("Restore Params", id="button-restoreParams", outline=True, color="info", className="me-2", n_clicks=0, style={"width": "195px"})], 
                                                      id="collapse-SVISubTable", is_open=False), style={"margin-top": "10px", "width": 200, "height": 100})], 
                                          style={'display': 'inline-block'})
    
    panelFig = html.Div(dbc.Row([
        dbc.Col(html.Div([dcc.Graph(id="graph-volSmile", figure=fig, style={'width': '1480px', 'height': '650px', "margin-left":0, "margin-right": 0})], id="graph-area")), 
    ...
    panelGrid = html.Div([dbc.Button("Show/Hide Grid", id="button-showVolGrid", outline=True, color="info", className="me-2", n_clicks=0, style={"width": "250px", "margin-bottom": "5px"}),
                          dbc.Collapse(gridTable, id="collapse-grid", is_open=True)], style={'display': 'inline-block', "width": "1780px"})
    ...
    panelGrid2 = html.Div([dbc.Button("Show/Hide Ref Grid", id="button-showRefGrid", outline=True, color="info", className="me-2", n_clicks=0, style={"width": "250px", "margin-top": "5px", "margin-bottom": "5px"}),
                           dbc.Collapse([refGridTable, html.H6(" "), html.H6("Change"), compareGridTable], 
                                        id="collapse-refGrid", is_open=False)], style={'display': 'inline-block', "width": "1780px"})
...
def createContentForward(undlName: str, memoryData: dict, dataCheck: bool=True):
    ...
    topBar = html.Div([html.Div([dbc.Button("Load", id="button-loadFwd", outline=True, color="danger", className="me-2", n_clicks=0),
                                 ...
                                 style={'display': 'inline-block'}),
                                 html.H6(" "),
                        html.Div("Status: Successfully Loaded", id="text-divPageStatus", style={"color": "black"}),
                        html.H6(" "),
                        html.Div(f"Dividend Last Update: {divLastUpdateUser} ({divLastUpdateTime}) / Repo Last Update: {repoLastUpdateUser} ({repoLastUpdateTime})", id="text-fwdLastUpdate", style={"color": "black"}),
                        html.Hr()])
...
@app.callback([Output("page-content", "children"), 
              ...
              [Input("url", "pathname"), 
               ...
               State("text-username", "children")], prevent_initial_call=True)
def render_page_content(pathname, undlName, memoryVolData, memoryFwdData, username):

    rowDataUndlInfo, rowDataMarketDataCheck, dataCheck = createUndlNameInfoRowData(undlName)

    if username == "Login as:":
        return html.Div(html.H4("Ready")), None, None, [], [], True

    if pathname == "/":
        return html.Div(html.H4("Ready")), memoryVolData, memoryFwdData, rowDataUndlInfo, rowDataMarketDataCheck, False
    elif pathname == "/forward":
        page, memoryFwdData = createContentForward(undlName, memoryFwdData, dataCheck)
        return page, memoryVolData, memoryFwdData, rowDataUndlInfo, rowDataMarketDataCheck, False
        ...
        return createContentAddUndl(undlName), memoryVolData, memoryFwdData, rowDataUndlInfo, rowDataMarketDataCheck, False
    elif pathname == "/setting":
        return createContentSetting(), memoryVolData, memoryFwdData, rowDataUndlInfo, rowDataMarketDataCheck, False
    elif pathname == "/otherMarketData":
        return createContentOtherMarketData(), memoryVolData, memoryFwdData, rowDataUndlInfo, rowDataMarketDataCheck, False
    elif pathname == "/vsfsetting":
        return createContentVSFBatch(), memoryVolData, memoryFwdData, rowDataUndlInfo, rowDataMarketDataCheck, False
    ...
    ), memoryVolData, memoryFwdData, rowDataUndlInfo, rowDataMarketDataCheck, False
...
def getOptionImpliedFwd(n_clicks, memoryData):
    ...
    if n_clicks:
        ...
        undlName = memoryData["undlName"]
        if get_exchange(undlName) == "HK" and get_undlType(undlName) == "Stock":
            optionChainData = getOptionChain_HKStock(undlName)
        else:
...
@app.callback([Output("text-divPageStatus", "children", allow_duplicate=True), 
               Output("text-divPageStatus", "style", allow_duplicate=True)], 
              [Input("confirm-saveFwd", "submit_n_clicks"), 
               Input("undlName-dropdown", "value"), 
               State("table-div", "rowData"), 
               State("table-repo", "rowData"),
               State("memory", "data")], prevent_initial_call=True)
def uploadDiv(submit_n_clicks, undlName, divData, repoData, username):
    if submit_n_clicks:
        
        divPanel = {"undlName": undlName, 
                    "lastUpdate": username["username"],
                    "lastUpdateTime": datetime.strftime(datetime.now(), "%Y-%m-%dT%H:%M:%S"),
        ...
        if check == 0: 
            return "Error in date string", {"color": "black"}
        divStatus = upload_dividend(divPanel)

        repoStatus = upload_repo(repoPanel)

        if divStatus["status"] != "Div uploaded" or repoStatus["status"] != "Repo uploaded":
            return f"Status: {divStatus['status']} / {repoStatus['status']}", {"color": "red"}
        else:
            return f"Status: {divStatus['status']} / {repoStatus['status']}", {"color": "green"}

    else:

        return "Status: Successfully Loaded", {"color": "black"}
...
@app.callback([Output("table-fwdUndlInfo", "rowData", allow_duplicate=True),
               ...
               Output("text-divPageStatus", "children", allow_duplicate=True),
               Output("text-divPageStatus", "style", allow_duplicate=True),],
...
def displayMemoryFwdData(memoryData):
    ...
    return dataUndlinfo, df_div.to_dict("records"), df_repo.to_dict("records"), memoryData["divSwapData"], memoryData["fwdData"], memoryData["repoTableData"], "Status: Successfully Loaded", {"color": "black"}
...
def displayMemoryVolData(memoryData, maturity):
    ...
    return undlInfo, textAnchor, textAnchorDate, SVIData, maturities, maturity, volGrid, is_open, repoTableData, is_open_SVIS, style, SVISData, volSurFit_disabled, SVISubTable_open, SVITable_open, columnDefs, "", refGridRowData, is_open_refSVI, refSVIRowData, styleDivRepoFitted, compareGrid
...
def changeMaturity(m, maturities, memoryData, subTableData, figure, rowData_SVI, rowData_Grid):
    ...
    result = [dcc.Graph(id="graph-volSmile", figure=fig, style={'width': '1480px', 'height': '650px', "margin-left": 0, "margin-right": 0, "margin-top":0 , "margin-bottom": 0}), 
              subTableData, 
              subTableData2, 
              memorySubParams, 
              selected_SVI, 
...
@app.callback(Output("memory-vol", "data", allow_duplicate=True), 
              [Input("SVITable", "cellValueChanged"), 
               ...
               State("SVITable", "rowData"),
               State("memory", "data")], prevent_initial_call=True)
def updateSVITable(cellValueChanged, memoryData, SVIData, credential):
    ...
    volSurfaceSVI = {"undlName": memoryData["undlName"], 
                     "anchor": anchor, 
                     "anchorDate": anchorDate, 
                     "lastUpdate": credential["username"],
                     "lastUpdateTime": datetime.strftime(datetime.now(), "%Y-%m-%dT%H:%M:%S"),
...
def changeVolUndlInfo(cellValueChanged, memoryData):
    ...
    if cellValueChanged != None:
        ...
        if cellValueChanged["colId"] == "Spot Ref":
            ...
            return memoryData, disabled, disabled
...
def addMaturity2(n_clicks, maturity, memoryData):
    ...
    if n_clicks:
        ...
        if maturity in memoryData["maturities"]:
            ...
            volSurfaceMaturities = list(volSurfaceSVI[volTag].keys())
            m1, m2 = getTwoClosestValuesFromList(maturity, volSurfaceMaturities)
            params = volSurfaceSVI[volTag][m1]
            if volTag == "SVI-JW":
                volSurfaceSVI[volTag][maturity] = {"vol": params["vol"], 
                                                   "skew": params["skew"], 
                                                   "pWing": params["pWing"], 
                                                   "cWing": params["cWing"], 
                                                    "minVol": params["minVol"], 
                                                    "tau": tau, "forward": forward}
                volSurfaceSVI_backup[volTag][maturity] = {"vol": params["vol"], 
                                                   "skew": params["skew"], 
                                                   "pWing": params["pWing"], 
                                                   "cWing": params["cWing"], 
                                                    "minVol": params["minVol"], 
            ...
            volSurfaceSVI_backup[volTag] = sortDictByKey(volSurfaceSVI_backup[volTag])
            ...
            memoryData["maturities"] = maturities
            memoryData["volSurfaceSVI"] = volSurfaceSVI
            memoryData["volSurfaceSVI_backup"] = volSurfaceSVI_backup
...
@app.callback(Output("text-settingPageStatus", "children", allow_duplicate=True), 
              [Input("button-saveSetting", "n_clicks")], prevent_initial_call=True)
def saveSetting(n_clicks):
...
@app.callback([Output("text-volPageStatus", "children", allow_duplicate=True), 
              ...
              [Input("confirm-uploadVol", "submit_n_clicks"), 
               State("memory-vol", "data"), 
               State("text-volPageStatus", "children"),
               State("memory", "data")], prevent_initial_call=True)
def uploadVolSurfConfirmed(submit_n_clicks, memoryData, statusChildren, username):

    if submit_n_clicks:
        volSurfaceSVI = memoryData["volSurfaceSVI"]
        volSurfaceSVI["lastUpdate"] = username["username"]
...
@app.callback(Output("memory-vol", "data", allow_duplicate=True), 
              [Input("button-fitVolSurf", "n_clicks"), 
               State("memory-vol", "data"),
               State("memory", "data")], prevent_initial_call=True)
def fitVolSurface(n_clicks, memoryData, credential):
        ...
        if memoryData != None:
            ...
            if "volData" in memoryData.keys():
                ...
                if memoryData["volData"]["call"]["bid"] != {} and memoryData["volData"]["put"]["bid"] != {}:
                    ...
                    result = fit_volSurfaceSVI(volModel=volModel, volData=memoryData["volData"], repoFitted=get_repo(undlName), username=credential["username"])
...
@app.callback([Output("text-username", "children", allow_duplicate=True), 
               Output("modal-login", "is_open", allow_duplicate=True),
               Output("memory", "data")],
              [Input("button-modal-login", "n_clicks"),
               State("input-username", "value"),
               State("input-password", "value")], prevent_initial_call=True)
def login(n_clicks, username, password):

    if n_clicks:
        if username in ["isaaclaw", "admin"]:
            return "Login as: "+username, False, {"username": username}
    
    return "Login as:", True, None
